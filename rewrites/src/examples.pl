...
"Registry":
-----------
M=seq([send(m,e_pid(p1),r),send(m,e_pid(p2),r),recv(m,x1),send(m,e_pid(r),m)]), R=seq([recv(r, x1),recv(r, x2),send(r,e_pid(m),a),recv(r, x3)]),
...
% Two loops with send to variable
P1A=seq([send(m,e_pid(Q), m)]),
P1B=seq([recv(m, x)]),
P2=seq([recv(P, id), send(P,e_var(id),P)]),
