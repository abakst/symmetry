...
data RemoteClient {- <p> -} = RemoteClient
       { remoteName :: ClientName
       , clientHome :: ProcessId{- <p> -}
       }
...
data Server {- <p> -} = Server
  ...
  , proxychan :: TChan (Process {- me < me ▹ send(p, PMessage) > -} ())
  , servers   :: TVar [ProcessId {- <p> -}]
  , spid      :: ProcessId {- <p> -}
  }

newServer :: [ProcessId{- <pid> -}] -> Process {- < me ▹ ε > -} (Server {- <p> -})
newServer pids = do
  pid <- getSelfPid
  liftIO $ do
    s <- newTVarIO pids
...
-- <<sendRemote
sendRemote :: Server {- <p> -} -> ProcessId {- <p> -} -> PMessage -> STM ()
sendRemote Server{..} pid pmsg = writeTChan proxychan (send pid pmsg)
-- >>

-- <<sendMessage
sendMessage :: Server {- <p> -} -> Client {- <p> -} -> Message -> STM ()
...
-- <<sendToName
sendToName :: Server {- <p> -} -> ClientName -> Message -> STM Bool
...
-- <<sendRemoteAll
sendRemoteAll :: Server {- <p> -} -> PMessage -> STM ()
...
-- <<broadcastLocal
broadcastLocal :: Server {- <p> -} -> Message -> STM ()
...
-- <<broadcast
broadcast :: Server {- <p> -} -> Message -> STM ()
...
-- <<tell
tell :: Server {- <p> -} -> LocalClient -> ClientName -> String -> IO ()
...
-- <<kick
kick :: Server {- <p> -} -> ClientName -> ClientName -> STM ()
...
checkAddClient :: Server {- <p> -} -> Client {- <p> -} -> STM Bool
...
-- <<runClient
runClient :: Server {- <p> -} -> LocalClient -> IO ()
...
-- <<handleMessage
handleMessage :: Server{- <p> -} -> LocalClient -> Message -> IO Bool
...
-- <<socketListener
socketListener :: Server{- <p> -} -> Int -> IO ()
...
-- <<proxy
proxy :: Server {- <p> -} -> Process {- me < me ▹ send(p, PMessage) > -} ()
...
chatServer port = do
  ...
  {- < q  ▹ μX. send(p, PMessage); μX > -}
  spawnLocal (proxy server)                              -- <2>
  {- < me ▹ μX. recv(PMessage); X -}
  forever $ do m <- expect; handleRemoteMessage server m -- <3>
-- >>

-- <<handleRemoteMessage
{-
handleRemoteMessage :: Server <p> -> PMessage
                    -> P me < me ▹ ε > ()
-}
