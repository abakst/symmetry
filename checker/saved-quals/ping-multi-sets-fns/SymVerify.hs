...
{-@ predicate SetMemEmpAxiom S I = ((~(Set_mem I (pidR1Blocked S))) => Set_emp (pidR1Blocked s)) @-}
{-@ predicate SetMemCardAxiom S = Set_emp (pidR1Blocked S) <=> pidR1NumBlocked S = 0 @-}
...
{-@ predicate BlockedDoneP1 S I = (Map_select (pidR1PcK S) (-1) + pidR1NumBlocked S == r1 S) &&
                                  ...
                                  ((Map_select (pidR1Pc s) i = (-1) && Map_select (pidR1PcK s) (-1) > 0) ||
                                   (Set_mem I (pidR1Blocked S) && Map_select (pidR1PcK S) 1 > 0)) @-}

{-@ assume assumption :: s:State -> i:Int -> i2:Int ->
                        {v:Bool | (Prop(v) <=>  ((SetMemEmpAxiom s i2 && SetMemCardAxiom s) &&
                                                 (BlockedDoneP0 s) &&
                                                 (BlockedDoneP1 s i))) }
@-}
