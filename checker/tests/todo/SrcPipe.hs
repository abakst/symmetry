...
class ( Symantics repr
      ...
      , SymTypes repr (Int -> Int) (Int, Pid RSing)
      , SymTypes repr Int (Pid RSing)
      , SymMatch repr () () (Process T_p)
      , SymTypes repr (Int -> Int) (Pid RSing)
      ) => PipeSem repr
...
pipe  = do me   <- self
           n    <- app any_nat tt
           head <- app3 init_pipe (lam $ \x -> plus (repI 1) x) n me
           send head (repI 0)
           sink
...
fix_init_pipe :: PipeSem repr
              => repr ((T_p -> Process T_p) -> T_p -> Process T_p)
fix_init_pipe  = lam $ \init_pipe -> lam $ \arg ->
                   ...
                   ifte (eq n (repI 0))
                     ...
                     (do role <- newRSing
                         ...
                         app init_pipe $ pair3 f (plus n (repI (-1))) new)
...
pipe_node :: PipeSem repr
          => repr ((Int->Int) -> (Pid RSing) -> Process ())
pipe_node  = lam $ \f -> lam $ \next ->
